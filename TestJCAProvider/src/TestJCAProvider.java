...
public class TestJCAProvider {
	...
	public static void main(String[] args) throws Exception {
		...
		// encrypt
		BigInteger codedBytes = test.encrypt(first.toByteArray(), pubKey, cipherHP);
		BigInteger codedBytes12 = test.encrypt(second.toByteArray(), pubKey, cipherHP);
		// product
		...
		// encrypt
		byte[] codedBytesBlock = test.encryptBlock(plainTextBlock.getBytes(), pubKey, cipher);
		...
		// decrypt
		byte[] encodedBytesBlock = test.decryptBlock(codedMessageBlock, privKey, cipher);
